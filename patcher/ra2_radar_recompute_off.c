...
int wmain(void) {
    ...
    if (!p) return 3;
    DWORD radar_object = 0; SIZE_T n = 0;
    if (!ReadProcessMemory(p, (LPCVOID)0x00A3D290, &radar_object, 4, &n) || n != 4 || !radar_object) {
        CloseHandle(p); return 4;
    }
    BYTE zero = 0;
    if (!WriteProcessMemory(p, (LPVOID)(uintptr_t)(radar_object + 0x2228), &zero, 1, &n) || n != 1) {
        CloseHandle(p); return 5;
    }
    /* Normal game path: ECX = player house; call 004F2E50. */
    DWORD player = 0;
    if (!ReadProcessMemory(p, (LPCVOID)0x00A35DB4, &player, 4, &n) || n != 4 || !player) {
        CloseHandle(p); return 6;
    }
    BYTE stub[16]; DWORD k = 0;
    stub[k++] = 0xB9; put32(stub + k, player); k += 4;
    stub[k++] = 0xE8; LPVOID mem = VirtualAllocEx(p, NULL, 0x1000,
        MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!mem) { CloseHandle(p); return 7; }
    put32(stub + k, 0x004F2E50 - ((DWORD)(uintptr_t)mem + k + 4)); k += 4;
    stub[k++] = 0xC3;
    BOOL ok = WriteProcessMemory(p, mem, stub, k, &n) && n == k;
    ...
    wprintf(L"RADAR_RECOMPUTE_OFF %ls\n", ok ? L"completed" : L"failed"); return ok ? 0 : 8;
}
