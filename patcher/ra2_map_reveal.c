...
int wmain(void){DWORD id=find_pid();if(!id)return 2;HANDLE p=OpenProcess(PROCESS_QUERY_INFORMATION|PROCESS_VM_OPERATION|PROCESS_VM_WRITE|PROCESS_CREATE_THREAD,FALSE,id);if(!p)return 3;BYTE stub[16];DWORD k=0;stub[k++]=0xB9;put(stub+k,0x008324E0);k+=4;stub[k++]=0xE8;LPVOID mem=VirtualAllocEx(p,NULL,0x1000,MEM_COMMIT|MEM_RESERVE,PAGE_EXECUTE_READWRITE);if(!mem){CloseHandle(p);return 4;}put(stub+k,(DWORD)(0x0055A120-((DWORD)(uintptr_t)mem+k+4)));k+=4;stub[k++]=0xC3;SIZE_T n=0;BOOL ok=WriteProcessMemory(p,mem,stub,k,&n)&&n==k;HANDLE t=NULL;if(ok)t=CreateRemoteThread(p,NULL,0,(LPTHREAD_START_ROUTINE)mem,NULL,0,NULL);if(t){ok=WaitForSingleObject(t,1000)==WAIT_OBJECT_0;CloseHandle(t);}VirtualFreeEx(p,mem,0,MEM_RELEASE);CloseHandle(p);wprintf(L"MAP_REVEAL %ls\n",ok?L"completed":L"failed");return ok?0:5;}
