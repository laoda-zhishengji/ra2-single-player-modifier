...
int wmain(int argc,wchar_t**argv){if(argc!=2||(_wcsicmp(argv[1],L"on")&&_wcsicmp(argv[1],L"off")))return 2;DWORD id=find_pid();if(!id)return 3;HANDLE p=OpenProcess(PROCESS_QUERY_INFORMATION|PROCESS_VM_OPERATION|PROCESS_VM_WRITE|PROCESS_CREATE_THREAD,FALSE,id);if(!p)return 4;DWORD target=_wcsicmp(argv[1],L"on")==0?0x0055A120:0x0055A070;BYTE stub[16];DWORD k=0;stub[k++]=0xB9;put(stub+k,0x008324E0);k+=4;stub[k++]=0xE8;LPVOID mem=VirtualAllocEx(p,NULL,0x1000,MEM_COMMIT|MEM_RESERVE,PAGE_EXECUTE_READWRITE);if(!mem){CloseHandle(p);return 5;}put(stub+k,target-((DWORD)(uintptr_t)mem+k+4));k+=4;stub[k++]=0xC3;SIZE_T n=0;BOOL ok=WriteProcessMemory(p,mem,stub,k,&n)&&n==k;HANDLE t=ok?CreateRemoteThread(p,NULL,0,(LPTHREAD_START_ROUTINE)mem,NULL,0,NULL):NULL;if(t){ok=WaitForSingleObject(t,1000)==WAIT_OBJECT_0;CloseHandle(t);}VirtualFreeEx(p,mem,0,MEM_RELEASE);CloseHandle(p);wprintf(L"MAP_SPYSAT_%ls %ls\n",argv[1],ok?L"completed":L"failed");return ok?0:6;}
